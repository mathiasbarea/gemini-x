...
export const useGeminiStream = (
    ...
    async (
      ...
      if (typeof query === 'string') {
        ...
        if (slashCommandResult) {
          ...
          if (handleShellCommand(trimmedQuery, abortSignal)) {
            return { queryToSend: null, shouldProceed: false };
          }
        } else if (isAtCommand(trimmedQuery)) {
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      const newSuccessfulCommandSaves = completedAndReadyToSubmitTools.filter(
        (t) =>
          t.request.name === 'write_file' &&
          t.status === 'success' &&
          (t.request.args['file_path'] as string).includes('.gemini/commands'),
      );

      if (newSuccessfulCommandSaves.length > 0) {
        triggerReload();
      }
